...
class Category(object):
    ...
    def __init__(self, name):
        self.name = name
        self.subcategories = []
        self.subcategories_dic = {} # maps first lower case word in subcategory's name to the subcategory itself
    ...
    def add_sub(self, subcategory):
        if subcategory not in self.subcategories:
            ...
            self.subcategories_dic[subcategory.name_lower] = subcategory
...
def find_cat_and_numwords_within_supercategory(word, supercategory):
    ...
    # Second value is the amount of words in the category's name
    category = supercategory.subcategories_dic[word.lower()]
...
#                print(ext, word, split[word])
                if is_in(categories, split[word]):
                    ...
                    word += extend(ext, split, word)
                    if is_in(subcategories2, split[word]) and self.subcategory.get_subcategories() != []: # Added the and statement so that adding words to the description that match a subcategory2 doesn't screw the algorithm
                        self.subcategory2, ext = find_cat_and_numwords_within_supercategory(split[word], self.subcategory)
                        word += extend(ext, split, word)
...
#
#line = '25.55 cva skin care this is a test'
#
##print(categories)
#
...
## Funtion that creates a csv output for google sheets and money pro
def write():
    # Load the accounts dictionary and the categories csv
    accounts_file = 'acc_dic.csv'
    categories_file = 'categories.csv'
    ...
    money_month = input('3-letter month abbreviation: ')
    ...
    for entry in entries:
        ...
        if entry.subcategory2 != '':
            ...
            elif entry.subcategory.get_name() == 'Car':
                category = 'Car: ' + str(entry.subcategory2)
            else:                
        ...
        f_money.write(money_month + ' ' + entry.day + ' 2021 at 12:00:00' + separator + # Day
